-- Templated by Airflow. Variables resolved at task render time:
--   {{ ti.xcom_pull(...).load_month }}, {{ var.value.gcs_bucket }}, {{ run_id }}

DECLARE load_month STRING DEFAULT '{{ ti.xcom_pull(task_ids="compute_run_params")["load_month"] }}';
DECLARE gcs_uri STRING DEFAULT 'gs://{{ var.value.gcs_bucket }}/{{ ti.xcom_pull(task_ids="compute_run_params")["gcs_object"] }}';
DECLARE load_month_date DATE DEFAULT PARSE_DATE('%Y-%m', load_month);
DECLARE load_id STRING DEFAULT CONCAT('composer_', '{{ run_id }}', '_', load_month);
